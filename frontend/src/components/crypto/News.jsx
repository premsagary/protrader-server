...
export default function News() {
  const { newsSym, setNewsSym } = useCryptoStore();
  const [articles, setArticles] = useState([]);
  const [loading, setLoading] = useState(false);

  const coin = CRYPTO_LIST.find((c) => c.sym === newsSym);
  const base = coin?.base || newsSym.replace('USDT', '');

  const fetchNews = async () => {
    setLoading(true);
    try {
      const data = await newsApi.fetchNews(base, `${base} cryptocurrency bitcoin`);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <select
          value={newsSym}
          onChange={(e) => setNewsSym(e.target.value)}
          style={{
            background: 'var(--bg3)',
        ...
        >
          ...
          <div className="flex gap-2.5 mt-1.5 text-xs" style={{ color: 'var(--text3)' }}>
            ...
            <span
              ...
              style={{
                color: +sentScore >= 0 ? 'var(--green-text)' : 'var(--red-text)',
              }}
            >
      ...
      {loading ? (
        ...
        <EmptyState message={`No news found for ${coin?.name || base}`} />
      ) : (
        articles.map((item, i) => (
          <NewsCard
