...
const TAB_MAP = [
  ...
  ['Stocks RoboTrade', 'Paper + live execution engine. Two-pass scan: Pass 1 collects BUY candidates, Pass 2 enters. Guards: Drawdown circuit-breaker (10/15/20%), Daily-loss cap (2% of equity), Varsity M2 Ch20 11-item binary gate (any fail → skip), Per-insert MAX_TRADES_PER_DAY=8 (binds mid-scan), Same-day dedup per symbol, MIN_EXPECTED_PROFIT_INR=₹200 floor on top of R:R=1.5, MAX_POSITIONS=10, 30-min post-SL cooldown.', 'var(--green-text)'],
            ...
            <Table
              ...
              rows={[
                ...
                [<b style={{ color: 'var(--amber-text)' }}>Varsity Binary Gate (REWRITTEN 2026-04-20)</b>,
                  <Code>per-candidate</Code>,
                  <>Any of 11 M2 Ch20 checklist items fails → <Code>scoreDayTrade</Code> returns <Code>null</Code>; candidate never surfaces</>,
                  <>Pivoted from score-threshold (<Code>MIN_DAY_TRADE_SCORE≥65</Code>) to Varsity's own binary checklist: priceAction · srContext · volumeConfirms · aboveDailyTrend · rrAcceptable · rrNetAcceptable · dowTrendNotDown · atrInTradeableBand · vixAcceptable · validSessionWindow · indicatorsAlign. Score still emitted as diagnostic field but is no longer a gate — <Code>MIN_DAY_TRADE_SCORE</Code> set to 0. Ranking of simultaneous candidates is handled by filter-chain + trigger-time FIFO.</>],
