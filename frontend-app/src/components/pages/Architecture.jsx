...
const TAB_MAP = [
  ...
  ['Stocks RoboTrade', 'Unified NSE stocks auto-trader (paper + live). 2026-04-20 consolidation iterated to completion today — final layout has ONE top-level tab with 12 sub-tabs: Overview · Agent · Scan · Candidates · Positions · Trade History · Analytics · Live Market · Chart · News · Portfolio · Scan Log. The standalone "Scan / DayTrade" tab and the standalone "Agent" tab were both folded in as sub-tabs (`stocks/scan`, `stocks/agent`); the `#daytrade` and `#agent` hash aliases still resolve to the matching sub-tab. The Agent sub-tab owns the master PAPER↔LIVE kill-switch + capital editor + Kite test-buy via components/common/TradingModeCard.jsx (moved out of Admin). Two-pass scan: Pass 1 collects BUY candidates, Pass 2 enters. Guards: Drawdown circuit-breaker (10/15/20%), Daily-loss cap (2% of equity), Varsity M2 Ch20+21 12-item binary gate (any fail → skip, includes ADX≥18 trend-strength), Per-insert MAX_TRADES_PER_DAY=8 (binds mid-scan), Same-day dedup per symbol, MIN_EXPECTED_PROFIT_INR=₹200 floor on top of R:R=1.5, MAX_POSITIONS=10, 30-min post-SL cooldown. 2026-04-20 v1.4.0: book-rules (agent/trading-rules.js) PROMOTED from diagnostic to gate via FILTERS.ENFORCE_BOOK_RULES_GATE — evaluateGateSubset runs net-new checks only (regime=NO_TRADE, losing patterns distinct from Varsity, structural NN) and a fail returns null. Full verdict still ships as `bookRules` for audit; gate verdict ships as `bookRulesGate`. The Scan sub-tab renders the intraday signal-generation surface (5-min VWAP Reclaim / Gap & Go / Breakout / Oversold Bounce detectors) — its output was never a separate pipeline, just a different timeframe of the same workflow.', 'var(--green-text)'],
  ...
  ['Admin',         'Read-only ops / diagnostics only (2026-04-20 slim-down). Pipeline controls, force-refresh buttons, LLM budget, users, logs, uptime. The master PAPER↔LIVE kill-switch + capital editor + ENABLE LIVE confirmation modal moved to Stocks RoboTrade > Agent (components/common/TradingModeCard.jsx) so the kill-switch lives next to the decisions it governs. The 5-layer GatesActiveBanner previously duplicated here was removed — it renders once in Stocks RoboTrade > Overview where the trader is actually looking.', 'var(--amber-text)'],
  ...
  └──────────────────────────────────────────────────────────────────────┘`}
          ...
          <SectionCard id="kite" icon="🔌" title="Kite Integration"
            ...
            <ol style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.7, margin: 0, paddingLeft: 20 }}>
              ...
              <li><b>⚠ DayTrade hardening suspended — 1-week A/B (2026-04-20 → ~2026-04-27)</b> — Reverted the 4-commit DayTrade hardening series (368524f preflight gates, d36c105 systemic context, be1e0af F&amp;O+pre-open+microstructure, b6ca9e2 hit-rate feedback). Day-1 live produced 5010/5013 rejections on F_MIN_SCORE with scores clustered at 30-32 (vs 100+ pre-hardening during the ₹14K paper run). Running pre-hardening scoring for one week to validate behavior, then deciding whether to re-apply full stack / partial / env-gated. Hardening commits retained in git history — one <Code>git revert</Code> away from restore.</li>
            </ol>
          </SectionCard>
          ...
          <SectionCard id="robotrade-guards" icon="🛡" title="RoboTrade Execution Guards"
            ...
            <Table
              ...
              rows={[
                ...
                [<b style={{ color: 'var(--amber-text)' }}>Varsity Binary Gate (REWRITTEN 2026-04-20)</b>,
                  <Code>per-candidate</Code>,
                  <>Any of 12 M2 Ch20+21 checklist items fails → <Code>scoreDayTrade</Code> returns <Code>null</Code>; candidate never surfaces</>,
                  <>Pivoted from score-threshold (<Code>MIN_DAY_TRADE_SCORE≥65</Code>) to Varsity's own binary checklist: priceAction · srContext · volumeConfirms · aboveDailyTrend · rrAcceptable · rrNetAcceptable · dowTrendNotDown · atrInTradeableBand · vixAcceptable · validSessionWindow · indicatorsAlign · <b>trendStrengthOk (ADX≥18, M2 Ch21)</b>. Score still emitted as diagnostic field but is no longer a gate — <Code>MIN_DAY_TRADE_SCORE</Code> set to 0. Ranking of simultaneous candidates is handled by filter-chain + trigger-time FIFO. Gap bars also carry a named <Code>gapType</Code> ∈ {'{ Common, Breakaway, Runaway, Exhaustion }'} (M2 Ch10) for UI context. <b>M2 Ch19 chart patterns</b> — Flag, Pennant, Symmetrical/Ascending/Descending Triangle, Rising/Falling Wedge, Double Top/Bottom, Head-and-Shoulders (+ inverse) — are detected on 5-min pivots (3-bar fractal) over the last ~60 bars. These are <b>corroborative, not gating</b>: bull-direction patterns nudge score +4..+6; bear patterns with reliability≥3 apply −5..−6 as a soft headwind. Full array + top pattern shipped in the pick payload.</>],
                [<b style={{ color: 'var(--red-text)' }}>Book-Rules GATE (PROMOTED 2026-04-20 v1.4.0)</b>,
                  <Code>per-candidate</Code>,
                  <>Net-new subset fails → <Code>scoreDayTrade</Code> returns <Code>null</Code>; same contract as Varsity. <Code>FILTERS.ENFORCE_BOOK_RULES_GATE=true</Code>.</>,
                  <>Promoted from diagnostic to hard gate (v1.1.0-book-rules-gate). The full evaluateAll verdict still ships as <Code>bookRules</Code> for audit (regime, 15-code rejection set, 0-100 quality A+..D, 20 non-negotiables NN01..NN20, losing patterns, entry predicates), but ONLY the <b>net-new subset</b> drives the null-return so Varsity-overlap checks don't double-gate. The gate runs three layers distinct from Varsity: (a) <b>Regime = NO_TRADE</b> only — state-age&gt;5s / VIX&gt;25 / |Nifty 15m move|&gt;1.5% / within first 10 min / last 30 min / circuit / earnings. (b) <b>Losing patterns</b> — chasing (&gt;0.30×ATR past trigger), vertical spike (≥3 green bars no pullback), counter-EMA stack (9&lt;20&lt;50 on long), big-gap-fade (gap&gt;2% + bear reversal), round-number trap (within 0.05% of ₹10/50/100/500 level by band). (c) <b>Structural NN</b> — NN01 SL present &amp; below entry · NN04 15:15 squareoff · NN05 avg-down disabled · NN06 widen-SL disabled · NN19 kill-switch not overridden · NN20 live mode requires paper-test pass. Skipped NN (enforced elsewhere): NN02/03/12/13/14 (constraint-engine), NN07/08/09/10/11/15/16/17/18 (Varsity checklist / picks preflight). Ctx built with real computed values: <Code>entryOffsetFromTriggerATR = (px − max(orHigh, pdHigh)) / ATR14</Code>, <Code>consecutiveGreenWithoutPullback</Code> scanned backward for close&gt;open &amp; low≥prevClose, <Code>roundNumberDistPct</Code> via price-banded nearest-round. Fail-open on exceptions — a bug in the module must never halt the mid-session pipeline; error surfaces on pick. Verdict shipped as <Code>bookRulesGate</Code>.</>],
