...
const TAB_MAP = [
  ...
  ['Scan',          'Intraday signal-generation surface — 5-min candles, VWAP Reclaim / Gap & Go / Breakout / Oversold Bounce detectors, scored in real-time. Separate cache + scoring engine via Unified Pipeline. (Was labeled "DayTrade" pre-2026-04-20 tab consolidation; tab id still `daytrade` for backwards-compat. Swing-scan output lives on Trade > Candidates because the Agent needs candidate context.)', '#ef4444'],
  ['Trade',         'Paper + live execution engine (relabeled from "Stocks RoboTrade" in the 2026-04-20 consolidation, with the old Agent page absorbed as the Agent sub-tab). Two-pass scan: Pass 1 collects BUY candidates, Pass 2 enters. The Agent sub-tab owns the master PAPER↔LIVE kill-switch + capital editor + Kite test-buy via components/common/TradingModeCard.jsx (moved out of Admin). Guards: Drawdown circuit-breaker (10/15/20%), Daily-loss cap (2% of equity), Varsity M2 Ch20+21 12-item binary gate (any fail → skip, includes ADX≥18 trend-strength), Per-insert MAX_TRADES_PER_DAY=8 (binds mid-scan), Same-day dedup per symbol, MIN_EXPECTED_PROFIT_INR=₹200 floor on top of R:R=1.5, MAX_POSITIONS=10, 30-min post-SL cooldown. 2026-04-20 v1.4.0: book-rules (agent/trading-rules.js) PROMOTED from diagnostic to gate via FILTERS.ENFORCE_BOOK_RULES_GATE — evaluateGateSubset runs net-new checks only (regime=NO_TRADE, losing patterns distinct from Varsity, structural NN) and a fail returns null. Full verdict still ships as `bookRules` (regime / entry / exit / risk / rejection / psych / 0-100 quality / losing patterns / 20 non-negotiables) for audit; gate verdict ships as `bookRulesGate`.', 'var(--green-text)'],
  ...
  ['Admin',         'Read-only ops / diagnostics only (2026-04-20 slim-down). Pipeline controls, force-refresh buttons, LLM budget, users, logs, uptime. The master PAPER↔LIVE kill-switch + capital editor + ENABLE LIVE confirmation modal moved to Trade > Agent (components/common/TradingModeCard.jsx) so the kill-switch lives next to the decisions it governs.', 'var(--amber-text)'],
