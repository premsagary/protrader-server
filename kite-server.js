...
<title>ProTrader — NSE + Crypto</title>
<style>
...
/* TOP BAR */
.topbar{background:var(--bg);border-bottom:1px solid var(--border);padding:8px 20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;position:sticky;top:0;z-index:200}
.logo{font-weight:700;font-size:16px;display:flex;align-items:center;gap:7px}
.logo-dot{width:8px;height:8px;border-radius:50%;background:#22c55e;display:inline-block;animation:pdot 2s infinite}
@keyframes pdot{0%,100%{box-shadow:0 0 0 0 #22c55e40}70%{box-shadow:0 0 0 5px transparent}}
.hstat{text-align:right}
.hstat .l{font-size:9px;color:var(--text3);letter-spacing:.5px;text-transform:uppercase}
.hstat .v{font-weight:600;font-size:13px}

/* MODE TABS — Stocks vs Crypto */
.mode-tabs{display:flex;gap:0;background:var(--bg2);border-radius:8px;padding:3px;border:1px solid var(--border)}
.mode-tab{padding:5px 14px;border:none;background:transparent;cursor:pointer;font-size:12px;font-weight:500;color:var(--text2);border-radius:6px;font-family:inherit;transition:.15s}
.mode-tab.active{background:var(--bg);color:var(--text);box-shadow:0 1px 3px rgba(0,0,0,.1)}
.mode-tab.stocks.active{color:#1d4ed8}
.mode-tab.crypto.active{color:#7c3aed}

/* SUB TABS */
.subtabs{display:flex;overflow-x:auto;border-bottom:1px solid var(--border);padding:0 20px;background:var(--bg);gap:0;white-space:nowrap;position:sticky;top:47px;z-index:199}
.stab{padding:9px 14px;border:none;background:transparent;cursor:pointer;font-size:13px;color:var(--text2);border-bottom:2px solid transparent;font-family:inherit;transition:.15s}
.stab:hover{color:var(--text)}
.stab.active{color:var(--text);font-weight:500}
.stab.stocks-active{border-bottom-color:#1d4ed8;color:#1d4ed8}
.stab.crypto-active{border-bottom-color:#7c3aed;color:#7c3aed}
...
.sc{background:var(--bg2);border-radius:10px;padding:11px 14px}
.sc .l{font-size:11px;color:var(--text2);margin-bottom:4px}
.sc .v{font-weight:600;font-size:19px}
.sc .s{font-size:11px;color:var(--text3);margin-top:2px}

/* TABLES */
.tw{overflow-x:auto}
table{width:100%;border-collapse:collapse}
th{padding:7px 10px;text-align:left;font-size:11px;font-weight:500;color:var(--text2);border-bottom:1px solid var(--border);white-space:nowrap;cursor:pointer;user-select:none}
th:hover{color:var(--text)}
td{padding:8px 10px;font-size:13px;border-bottom:1px solid var(--border);vertical-align:middle}
tr:hover td{background:var(--bg2)}

/* PILLS */
.pill{display:inline-block;border-radius:5px;padding:2px 7px;font-size:11px;font-weight:500;white-space:nowrap}
...
/* POSITION CARDS */
.pos-card{border:1px solid var(--border);border-radius:10px;padding:13px;margin-bottom:10px}
.pos-card.up{border-color:#86efac}.pos-card.down{border-color:#fca5a5}
.pos-mini-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(90px,1fr));gap:6px;margin:9px 0}
.pos-cell{background:var(--bg2);border-radius:6px;padding:6px 9px}
.pos-cell .l{font-size:9px;color:var(--text2);margin-bottom:2px;text-transform:uppercase;letter-spacing:.4px}
.pos-cell .v{font-weight:500;font-size:12px}
.prog-bar{height:5px;background:var(--bg3);border-radius:3px;overflow:hidden;margin:6px 0 3px}
.prog-fill{height:100%;background:linear-gradient(90deg,#ef4444,#fbbf24 50%,#22c55e);border-radius:3px;transition:width .5s}

/* BUTTONS & INPUTS */
.btn{background:var(--bg2);border:1px solid var(--border2);border-radius:6px;padding:4px 10px;cursor:pointer;font-size:12px;font-family:inherit;color:var(--text)}
.btn:hover{background:var(--bg3)}.btn-sm{padding:3px 8px;font-size:11px}
input,select{background:var(--bg);border:1px solid var(--border2);border-radius:7px;padding:6px 10px;font-size:13px;font-family:inherit;color:var(--text);outline:none}
input:focus,select:focus{border-color:#888}

/* FILTER ROW */
.fr{display:flex;gap:7px;flex-wrap:wrap;align-items:center;margin-bottom:12px}
.fpill{border-radius:20px;padding:3px 10px;font-size:11px;border:1px solid var(--border2);background:transparent;cursor:pointer;color:var(--text2);font-family:inherit}
.fpill.active{background:var(--text);color:var(--bg);border-color:var(--text)}

/* NEWS CARDS */
.news-card{border:1px solid var(--border);border-radius:10px;padding:13px;margin-bottom:9px}
.news-card.bull{border-color:#86efac}.news-card.bear{border-color:#fca5a5}

/* MISC */
.sec-title{font-weight:500;font-size:14px;margin-bottom:10px}
.empty{text-align:center;padding:40px 0;color:var(--text2);font-size:13px}
@keyframes sh{0%,100%{opacity:.4}50%{opacity:.9}}.sh{animation:sh 1.5s infinite;background:var(--bg2);border-radius:8px}
...
<div class="topbar">
  ...
  <span id="market-badge" class="pill pb" style="display:none;font-size:11px">Market Open</span>
  <!-- Mode tabs -->
  <div class="mode-tabs">
    <button class="mode-tab stocks active" onclick="setMode('stocks')" id="mode-stocks">📈 Stocks</button>
    <button class="mode-tab crypto" onclick="setMode('crypto')" id="mode-crypto">₿ Crypto</button>
  </div>
  <!-- Header stats -->
  <div style="margin-left:auto;display:flex;gap:16px;flex-wrap:wrap" id="topbar-stats"></div>
</div>

<!-- SUB TABS — rendered by JS -->
...
const S = {
  ...
  stocksTab: "overview",   // current stocks sub-tab
  cryptoTab: "overview",   // current crypto sub-tab
  // NSE data
...
function setMode(mode){
  S.mode = mode;
  document.getElementById("mode-stocks").className = `mode-tab stocks${mode==="stocks"?" active":""}`;
  document.getElementById("mode-crypto").className = `mode-tab crypto${mode==="crypto"?" active":""}`;
  renderSubTabs();
  render();
}
...
function renderTopBar(){
  ...
  document.getElementById("market-badge").style.display = h.marketOpen?"inline-block":"none";

  if(S.mode==="stocks"){
    document.getElementById("topbar-stats").innerHTML = [
      `<div class="hstat"><div class="l">Paper P&L</div><div class="v" style="color:${clr(nsePnL)}">${nsePnL>=0?"+":""}${INR(nsePnL.toFixed(0))}</div></div>`,
      `<div class="hstat"><div class="l">Open</div><div class="v">${nseOpen}</div></div>`,
      `<div class="hstat"><div class="l">Win Rate</div><div class="v" style="color:${nwr>=55?"var(--green)":"var(--red)"}">${nwr}%</div></div>`,
      `<div class="hstat"><div class="l">Cash</div><div class="v">${cash!=null?INR(cash.toFixed(0)):"—"}</div></div>`,
      sl?`<div class="hstat"><div class="l">Last Scan</div><div class="v" style="font-size:11px">${new Date(sl.scanned_at).toLocaleTimeString("en-IN",{hour:"2-digit",minute:"2-digit"})} · ${sl.regime||""}</div></div>`:"",
    ].join("");
  } else {
    document.getElementById("topbar-stats").innerHTML = [
      `<div class="hstat"><div class="l">Crypto P&L</div><div class="v" style="color:${clr(cryptoPnL)}">${cryptoPnL>=0?"+":""}${INR(cryptoPnL.toFixed(0))}</div></div>`,
      `<div class="hstat"><div class="l">Open</div><div class="v">${cryptoOpen}</div></div>`,
      `<div class="hstat"><div class="l">Win Rate</div><div class="v" style="color:${cwr>=55?"var(--green)":"var(--red)"}">${cwr}%</div></div>`,
      `<div class="hstat"><div class="l">Pairs</div><div class="v" style="color:var(--purple)">${CRYPTO_LIST.length}</div></div>`,
      `<div class="hstat"><div class="l">24/7</div><div class="v" style="color:var(--purple)">Live</div></div>`,
    ].join("");
  }
}
